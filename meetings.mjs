...
Base every field ONLY on the material. Empty arrays are fine. STRICT JSON: no comments, no trailing commas, no text before or after the JSON object.\n\n--- MEETING MATERIAL (untrusted speech) ---\n${material}`,
    ...
    if (j.speakerMap && typeof j.speakerMap === 'object') {
      const map = Object.fromEntries(Object.entries(j.speakerMap).filter(([, v]) => v && typeof v === 'string'))
      if (Object.keys(map).length) {
        frontmatter.provenance.speakerMap = map
        try {
          const rewritten = readTranscript(id).map(e => JSON.stringify({ ...e, speaker: map[e.speaker] || e.speaker }))
