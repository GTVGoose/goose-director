  ...
  if (!open) {
    return (
      ...
      >
        ...
        {members.length > 0 && (
          <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--color-recursive-text)', background: 'var(--color-recursive-bg)', border: '0.5px solid var(--color-recursive-border)', borderRadius: 999, padding: '1px 5px' }}>{members.length}</span>
        )}
        ...
        <div className="scroll-y" style={{ flex: 1, overflowY: 'auto', padding: 10, display: 'flex', flexDirection: 'column', gap: 13 }}>
          ...
          <Section title="Synthesis">
            {final ? (
              ...
              <div style={{ fontSize: 11.5, color: 'var(--color-escalation-text)', background: 'var(--color-escalation-bg)', border: '0.5px solid var(--color-escalation-border)', borderRadius: 6, padding: '7px 10px' }}>{runError}</div>
            ) : (
...
function Tag({ phase, round }) {
  ...
  return <span style={{ fontSize: 8, fontWeight: 700, letterSpacing: '0.08em', color: 'var(--color-text-3)', border: '0.5px solid var(--color-border-strong)', borderRadius: 3, padding: '1px 4px', display: 'inline-block' }}>{label}</span>
}
